...
# Usar siempre ruta absoluta para el volumen (el script hace cd después; ./Development se resolvería mal)
ISAAC_ROS_DEV_DIR="$(cd "$ISAAC_ROS_DEV_DIR" && pwd)"

# Opcional: si el siguiente argumento es "--", el resto es el comando a ejecutar en el contenedor
CONTAINER_CMD=()
if [[ "$1" == "--" ]]; then
    shift 1
    CONTAINER_CMD=("$@")
...
if [ "$(docker ps -a --quiet --filter status=running --filter name=$CONTAINER_NAME)" ]; then
    ...
    if [ ${#CONTAINER_CMD[@]} -gt 0 ]; then
        docker exec -i -t -u admin --workdir /workspaces/isaac_ros-dev $CONTAINER_NAME "${CONTAINER_CMD[@]}"
    else
...
# Usar --runtime nvidia solo si está explícitamente habilitado Y el toolkit está configurado.
# Por defecto NO se usa GPU, para evitar ERROR_LIBRARY_NOT_FOUND cuando los drivers no están o fallan.
RUNTIME_NVIDIA=()
if [ -n "${USE_NVIDIA_GPU:-}" ] && [ "${USE_NVIDIA_GPU}" != "0" ] && docker info 2>/dev/null | grep -q nvidia; then
    RUNTIME_NVIDIA=("--runtime" "nvidia")
    print_info "Usando runtime NVIDIA (USE_NVIDIA_GPU está definido)."
else
    if [ -z "${USE_NVIDIA_GPU:-}" ] || [ "${USE_NVIDIA_GPU}" = "0" ]; then
        print_info "Contenedor sin GPU (por defecto). Para usar GPU: export USE_NVIDIA_GPU=1 y tener drivers + nvidia-container-toolkit."
    else
        print_warning "USE_NVIDIA_GPU está definido pero NVIDIA runtime no está disponible; contenedor sin GPU."
    fi
fi
